...
DROP VIEW IF EXISTS vw_row_counts;
CREATE VIEW vw_row_counts as 
SELECT 'src_beneficiary_summary' AS object_name, count(*) AS row_count FROM src_beneficiary_summary
UNION ALL SELECT 'new_beneficiary_summary', count(*) FROM new_beneficiary_summary
UNION ALL SELECT 'beneficiary_summary_orphans', count(*) FROM beneficiary_summary_orphans
UNION ALL SELECT 'beneficiary_summary_extra_rows', count(*) FROM beneficiary_summary_extra_rows
UNION ALL SELECT 'beneficiary_summary_mismatch_summary', count(*) FROM beneficiary_summary_mismatch_summary
UNION ALL SELECT 'src_carrier_claims', count(*) FROM src_carrier_claims
UNION ALL SELECT 'new_carrier_claims', count(*) FROM new_carrier_claims
UNION ALL SELECT 'carrier_claims_orphans', count(*) FROM carrier_claims_orphans
UNION ALL SELECT 'carrier_claims_extra_rows', count(*) FROM carrier_claims_extra_rows
UNION ALL SELECT 'carrier_claims_mismatch_summary', count(*) FROM carrier_claims_mismatch_summary
UNION ALL SELECT 'v_beneficiary_summary_mismatches', count(*) FROM v_beneficiary_summary_mismatches
UNION ALL SELECT 'v_carrier_claims_mismatches', count(*) FROM v_carrier_claims_mismatches;

-- Export the high-level summaries
COPY beneficiary_summary_mismatch_summary TO 'E:\Data Eng Exercise\bene_summary.parquet' (FORMAT PARQUET);
COPY carrier_claims_mismatch_summary TO 'E:\Data Eng Exercise\claims_summary.parquet' (FORMAT PARQUET);

-- Export the side-by-side "Evidence" views
-- Even with 20,000+ rows, these will be very small files
COPY (SELECT * FROM v_beneficiary_summary_mismatches) TO 'E:\Data Eng Exercise\bene_diffs.parquet' (FORMAT PARQUET);
COPY (SELECT * FROM v_carrier_claims_mismatches) TO 'E:\Data Eng Exercise\claims_diffs.parquet' (FORMAT PARQUET);

-- Export the Orphans/Extras
COPY beneficiary_summary_orphans TO 'E:\Data Eng Exercise\bene_orphans.parquet' (FORMAT PARQUET);
COPY carrier_claims_extra_rows TO 'E:\Data Eng Exercise\claims_extras.parquet' (FORMAT PARQUET);


-- For readable charts (State, Sex, Race)
COPY lookup_state TO 'E:\Data Eng Exercise\lookup_state.parquet' (FORMAT PARQUET);
COPY lookup_sex TO 'E:\Data Eng Exercise\lookup_sex.parquet' (FORMAT PARQUET);
COPY lookup_race TO 'E:\Data Eng Exercise\lookup_race.parquet' (FORMAT PARQUET);
-- The "Magic" Table: This maps technical names (SP_ALZHDMTA) to labels (Alzheimer's)
COPY lookup_variable_labels TO 'E:\Data Eng Exercise\lookup_variable_labels.parquet' (FORMAT PARQUET);
-- High-level row counts for your "Migration Scorecard" gauge
COPY (SELECT * FROM vw_source_counts) TO 'E:\Data Eng Exercise\summary_counts.parquet' (FORMAT PARQUET);
-- Final pieces of the "Missing Records" puzzle
COPY beneficiary_summary_extra_rows TO 'E:\Data Eng Exercise\bene_extras.parquet' (FORMAT PARQUET);
...
CREATE OR REPLACE VIEW v_carrier_line1_audit AS
...
-- Export this "Cleaned" audit to a fresh Parquet file
COPY (SELECT * FROM v_carrier_line1_audit) 
TO 'E:\Data Eng Exercise\carrier_audit_valid_only.parquet' (FORMAT PARQUET);
...
COPY (SELECT * FROM audit_carrier_claims) TO 'E:\Data Eng Exercise\audit_carrier_claims.parquet' (FORMAT PARQUET);
...
COPY (SELECT * FROM audit_beneficiary_summary) TO 'E:\Data Eng Exercise\audit_beneficiary_summary.parquet' (FORMAT PARQUET);
...
-- Move all CTE definitions to the top
...
COPY (SELECT * FROM vw_sigma_analysis) TO 'E:\Data Eng Exercise\vw_sigma_analysis.parquet' (FORMAT PARQUET);

SELECT 
    table_name
    ,column_name
FROM 
    data_eng.main.vw_db_schema
WHERE 
    table_name in ('audit_beneficiary_summary', 'audit_carrier_claims')
ORDER BY 
    table_name;
...
COPY (SELECT * FROM vw_sigma_analysis_columns) TO 'E:\Data Eng Exercise\vw_sigma_analysis_columns.parquet' (FORMAT PARQUET);
...
COPY (SELECT * FROM audit_carrier_financials) TO 'E:\Data Eng Exercise\audit_carrier_financials.parquet' (FORMAT PARQUET);
...
COPY (SELECT * FROM audit_beneficiary_financials) TO 'E:\Data Eng Exercise\audit_beneficiary_financials.parquet' (FORMAT PARQUET);

CREATE OR REPLACE VIEW vw_financial_differences AS
WITH agg AS (
    SELECT
        DESYNPUF_ID
        , CLM_ID
...
COPY vw_financial_differences TO 'E:\Data Eng Exercise\vw_financial_differences.parquet' (FORMAT PARQUET);
...
WITH flat AS (
    ...
    FROM data_eng.main.vw_financial_differences
)
SELECT
    unnest.metric_name,
...
COPY audit_claim_financial_fields TO 'E:\Data Eng Exercise\audit_claim_financial_fields.parquet' (FORMAT PARQUET);
...
COPY vw_financial_differences_bene TO 'E:\Data Eng Exercise\vw_financial_differences_bene.parquet' (FORMAT PARQUET);
...
GROUP BY unnest.metric_name
ORDER BY total_abs_delta DESC;
COPY audit_beneficiary_financial_fields TO 'E:\Data Eng Exercise\audit_beneficiary_financial_fields.parquet' (FORMAT PARQUET);

select distinct column_name from data_eng.main.vw_sigma_analysis_columns;
